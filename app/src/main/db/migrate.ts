...
const MIGRATIONS: Record<number, (db: MigrationDb) => void> = {
  ...
  // v16 — canonical Hacker News Source over the public HN Search API. As with
  // v15, existing Scouts receive no scout_sources row: an app update never
  // grants outbound Source Access implicitly.
  16: (db) => {
    if (hasTable(db, "sources") && hasTable(db, "source_access")) {
      db.exec(`
        INSERT OR IGNORE INTO sources (
